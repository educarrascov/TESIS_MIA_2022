;; velocidades
;; Botes: 12.8 mt/s = 46 Km/h = 0,8 puntos/tick          rango: 7 a 8
...
to main-simulation
  ...
  ;ask turtles with [tipo = "drone" and subtipo = "inspector"][
    ...
    ask turtle id-destino [
      ...
      ifelse (F-rango round(tmp-xcor-drone) round(tmp-ycor-drone) xcor ycor 2)[  ; Drone llegó al objeto ?
        ...
        ifelse (tmp-id-drone - q-botes = tmp-id-destino) [ ; regresó a la torre de control ?
  ...
  if (b-actualiza-estadisticas)[
    ...
    set prom-dr precision (total-dr / q-botes) 1
    set prom-tpo-vuelo precision (total-tpo-vuelo / q-botes) 1
    set prom-objetivos precision (total-objetivos / q-botes) 1
    set prom-consumo precision (total-consumo / q-botes) 1
    ifelse q-botes > 1 [
...
to setup_globals
  ...
  set id-primer-drone 1 + q-embarcaciones * 2 + q-botes
  ...
  set k-consumo-por-tick 0.0185 ; equivalente a autonomía de a 5400 ticks, si 1 tick = 1 seg -> 90 minutos
  set k-recarga-por-tick 0.0417 ; equivalente a recarga en 2400 ticks, si 1 tick = 1 seg -> 40 minutos
  set k-holgura-regreso-a-torre 10
  set k_almacenamiento-minimo 100
  ...
  array:set x-initial-position-drone 0 -18
  array:set x-initial-position-drone 1 18
  ...
  set q-visibles 1 + q-botes + q-embarcaciones
end
...
to setup_turtles
  ...
  create-turtles q-botes [
    set tipo "drone"
    set subtipo "initial"
    set color gray
    set size q-size-drone
    ;set xcor q-botes / -2 * q-size-drone + (who + 1 - q-old-turtles + 0.5) * q-size-drone
  ...
  create-turtles q-botes [
    ...
    set color F-color-ruta (who + 1 - q-old-turtles - q-botes)
    set size q-size-drone
    ;set xcor q-botes / -2 * q-size-drone + (who + 1 - q-old-turtles - q-botes + 0.5) * q-size-drone * 3
    ;set ycor q-size-drone * -1.5
    let tmp-xcor -1
    let tmp-ycor -1
    ask turtle (who - q-botes)[
      set tmp-xcor xcor
      set tmp-ycor ycor
    ]
    set xcor tmp-xcor
    set ycor tmp-ycor
    set shape "boatfiscal3"

    ; primera asignación
    set id-origen (who - q-botes) ; El initial correspondiente a cada drone, el destino se determinará por distancia
    set id-destino -1
    ...
    set objetivos 0
    set consumo 0
    set velocidad random(3) / 10.0 + 3.2 ; Entre 3.2 a 3.4 / Se divide por 10 al aplicar
    ;set almacenamiento random(101) + 0 ; de 0 a 100
    set almacenamiento 50 ; fijo al 50%
  ...
  create-turtles q-botes [
    set tipo "drone"
    set subtipo "store"
    set color F-color-ruta (who + 1 - q-old-turtles - q-botes * 2)
    ;set size q-size-drone
    set size q-size-drone / 5 * 3
    set shape "store 000"
    ;set xcor array:item x-initial-position-store (who - q-embarcaciones * 2 - q-botes * 2 - 1)
    ;set ycor array:item y-initial-position-store (who - q-embarcaciones * 2 - q-botes * 2 - 1)
    set xcor [xcor] of turtle (who - q-botes)
    set ycor ([ycor] of turtle (who - q-botes)) - q-size-drone / 5 * 4;
  ...
  create-turtles q-botes [
    ...
    ;set label-color F-color-ruta (who + 1 - q-old-turtles - q-botes * 3)
    set label-color black
    set size q-size-drone
    set shape "nothing"
    ;set xcor array:item x-initial-position-label-store (who - q-embarcaciones * 2 - q-botes * 3 - 1)
    ;set ycor array:item y-initial-position-label-store (who - q-embarcaciones * 2 - q-botes * 3 - 1)
    set xcor ([xcor] of turtle (who - q-botes * 2)) - q-size-drone / 9
    set ycor ([ycor] of turtle (who - q-botes * 2)) - q-size-drone / 8 * 7
    ifelse administracion-energia [
      show-turtle
    ]
    [
...
to F-asigna-mas-cercano-OBJ
  ...
  ask turtles with [tipo = "drone" and subtipo = "inspector" and estado != "perdido"][
    ...
    if estado-energia = "volviendo-a-recargar" [
      set tmp-id-objeto-a-asignar who - q-botes
      set b-entro true
    ]
    ifelse (b-entro)[
      set estado "en-ruta"
...
to F-despliega-almacenamiento [id-drone]
  ...
  ask turtles with [who = id-drone + q-botes * 2][
    set label (word (precision tmp-almacenamiento 0) "%")
  ]
  ; shape
  ask turtles with [who = id-drone + q-botes][
...
false
0
Circle -7500403 true true 90 0 148
Circle -7500403 true true 60 0 148
Circle -7500403 true true 60 150 148
Circle -7500403 true true 90 150 148
Rectangle -7500403 true true 60 60 240 240
Circle -16777216 true false 96 21 108
Circle -16777216 true false 96 171 108
Rectangle -7500403 true true 120 0 180 15
Rectangle -7500403 true true 120 285 180 300
Rectangle -16777216 true false 90 75 210 225
Rectangle -7500403 true true 90 75 210 105
Rectangle -7500403 true true 90 135 210 165
Rectangle -7500403 true true 90 195 210 225

boatfiscal3-border
false
1
Circle -7500403 true false 90 0 148
Circle -7500403 true false 60 0 148
Circle -7500403 true false 60 150 148
Circle -7500403 true false 90 150 148
Rectangle -7500403 true false 60 60 240 240
Circle -16777216 true false 96 21 108
Circle -16777216 true false 96 171 108
Rectangle -7500403 true false 120 0 180 15
Rectangle -7500403 true false 120 285 180 300
Rectangle -16777216 true false 90 90 210 240
Rectangle -7500403 true false 90 135 210 165
Rectangle -7500403 true false 90 195 210 225
Line -2674135 true 60 60 60 240
Line -2674135 true 240 60 240 240
Line -2674135 true 90 105 210 105
Line -2674135 true 90 225 210 225
Line -2674135 true 90 75 210 75
Line -2674135 true 90 195 210 195
Line -2674135 true 90 165 210 165
Line -2674135 true 90 135 210 135
Circle -2674135 false true 90 15 120
Circle -2674135 false true 90 165 120
Rectangle -7500403 true false 90 75 210 105
Rectangle -7500403 true false 90 195 210 225
Rectangle -16777216 true false 90 105 210 135
Rectangle -16777216 true false 90 165 210 195
Line -2674135 true 90 75 210 75
Line -2674135 true 90 105 210 105
Line -2674135 true 90 165 210 165
Line -2674135 true 90 195 210 195
Line -2674135 true 90 105 90 135
Line -2674135 true 210 165 210 195
Line -2674135 true 90 165 90 195
Line -2674135 true 210 105 210 135
